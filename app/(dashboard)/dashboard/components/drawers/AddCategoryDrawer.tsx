...
export default function AddCategoryDrawer({ onClose }: AddCategoryDrawerProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative bg-[#001A6E] text-white w-full sm:w-[340px] h-full p-5 shadow-2xl rounded-l-2xl"
      >
        {/* Header */}
          ...
          {loading ? (
            ...
            <ul className="space-y-1">
              {categories.map((cat) => (
                <li
                  key={cat.id}
                  className="flex justify-between items-center bg-[#010c32] hover:bg-gray-700 p-2 rounded-md text-xs "
                >
                  <div className="flex flex-col">
                    <span className="font-medium">{cat.name}</span>
                    <span className="text-gray-400 text-[10px]">
                      مادر: {getParentName(cat.parentId)}
                    </span>
                  </div>
                  <button
